...
export default {
  ...
  created: function() {
    this.item_id = this.$router.currentRoute.params.id;
    this.getData();
  },
  methods: {
    getData() {
      this.$http
        .get(utils.apiBaseUrl + "lecturers/" + this.item_id)
        .then(resp => {
          this.lecturer = resp.data.data.lecturer;
      ...
      for (var hour in data) {
        ...
        for (var day in days) {
          ...
          for (var exam in exams) {
            if (
              exams[exam].toLowerCase().indexOf(keyword.toLowerCase()) === -1
            ) {
